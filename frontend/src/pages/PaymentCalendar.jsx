...
export default function PaymentCalendar({ initialWeek, initialYear }) {
    ...
    api.paymentCalendar
      ...
      .then((rows) => {
        ...
        (rows || []).forEach((row) => {
          ...
          if (row.category === 'expense_plan_unmatched') {
            map[cellKey(row.week_number, row.category, sub)] = {
              plan: parseFloat(row.plan || 0),
              fact: parseFloat(row.fact || 0),
              id: row.id,
            };
            return;
          }
  ...
  useEffect(() => {
    api
      .get(`/api/payment-calendar/planned-expenses-by-week?year=${year}`)
      .then((r) => {
        setExpenseByWeek(r && typeof r === 'object' && !Array.isArray(r) ? r : {});
      })
      .catch(() => setExpenseByWeek({}));
  }, [year]);
