/**
 * Страница склада.
 * Остатки по размерам и партиям. Отгрузка с проверкой ship_qty <= warehouse_qty.
 */
...
const BTN_DANGER = 'bg-red-600 hover:bg-red-500 text-white';
...
export default function Warehouse() {
  ...
  const setShipQty = (id, value) => {
    setShipInputs((prev) => ({ ...prev, [id]: value }));
  };
              ...
              <table className="w-full min-w-[700px]">
                <thead>
                  <tr className="border-b border-white/20" style={{ background: 'rgba(255,255,255,0.05)' }}>
                    <th className="text-left px-4 py-3 text-sm font-medium text-white col-print-hide">Партия</th>
                    <th className="text-left px-4 py-3 text-sm font-medium text-white">Модель</th>
                    <th className="text-left px-4 py-3 text-sm font-medium text-white">Размер</th>
                    <th className="text-right px-4 py-3 text-sm font-medium text-white">Остаток</th>
                    <th className="text-left px-4 py-3 text-sm font-medium text-white no-print">Отгрузить</th>
                    <th className="text-left px-4 py-3 text-sm font-medium text-white no-print">Действие</th>
                  </tr>
                </thead>
                <tbody>
                  {list.map((row) => (
                    <tr key={row.id} className="border-b border-white/10 hover:bg-white/5">
                      <td className="px-4 py-3 text-white col-print-hide">{batchCode(row)}</td>
                      <td className="px-4 py-3 text-white">{modelName(row)}</td>
                      <td className="px-4 py-3 text-white">{sizeName(row)}</td>
                      <td className="px-4 py-3 text-right font-medium text-white">{warehouseQty(row)}</td>
